...
# Main equations
term_p = (1 + 2*epsilon)*(p.dx2) - np.sqrt(1+2*delta)*q.dy2
term_q = np.sqrt(1 + 2*delta)*(p.dx2)  + q.dy2
...
pos = ax.imshow(np.transpose(p.data[it,:,:]), cmap="seismic", vmin=-amax2, vmax=+amax2, extent=plt_extent)
